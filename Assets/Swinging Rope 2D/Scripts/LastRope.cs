...
public class LastRope : MonoBehaviour
{
    ...
    PlayerMovementOnRope playerMovementOnRope;
    ...
    void Start()
    {
        ...
        playerController = player.GetComponent<PController>();
    ...
    void Update()
    {
        ...
        {
            canConnect = true;
            if (!playerController.alreadyConnected && dismountRope)
            {
                dismountRope = false;
        ...
        if (connected)
        {
            ...
            playerController.alreadyConnected = true;
        ...
        else if (resetGravity && playerController.IsGrounded())
        {
            resetGravity = false;
        ...
        if (Input.GetKeyDown(KeyCode.Space) && connected)
        {
            ...
            playerController.Jump(playerController.jumpHeight);

            // Play the correct animation after dismounting
        ...
        if (Input.GetKey(KeyCode.DownArrow) && positionNumber == newPlayerRopePositions.Count - 1 - numberOfTransformsPerSegment
    ...
    public void DismountRope()
    {
        ...
        connected = false;
        playerController.alreadyConnected = false;
        dismountRope = true;
        idleAnimation = false;
    ...
    void HandlePlayerPosition()
    {
        ...
        if ((Input.GetKeyDown(KeyCode.UpArrow) ||
            ((Input.GetKey(KeyCode.UpArrow) || Input.GetAxisRaw("Vertical") == 1) && holdButtonToMove))
            && positionNumber > 0)
        {
            ...
            ((Input.GetKey(KeyCode.DownArrow) || Input.GetAxisRaw("Vertical") == -1) && holdButtonToMove))
            && positionNumber < newPlayerRopePositions.Count - 1 - numberOfTransformsPerSegment)
        {
    ...
    void HandlePlayerState()
    {
        ...
        if (playerIsMoving)
        {
            ...
            if (player.transform.localPosition.y > newPlayerRopePositions[positionNumber].localPosition.y && !fallingAnimation)
            {
                ...
                playerMovementOnRope = PlayerMovementOnRope.falling;
                playerController.HandleRopeAnimationsAndSoundEffects(playerMovementOnRope);
            }
            ...
            else if (player.transform.localPosition.y < newPlayerRopePositions[positionNumber].localPosition.y && !climbingAnimation)
            {
                ...
                // Set the movement state
                playerMovementOnRope = PlayerMovementOnRope.climbing;
                playerController.HandleRopeAnimationsAndSoundEffects(playerMovementOnRope);
            }
        ...
        else if(!idleAnimation)
        {
            ...
            // Set the movement state
            playerMovementOnRope = PlayerMovementOnRope.notMoving;
            playerController.HandleRopeAnimationsAndSoundEffects(playerMovementOnRope);
        }        
    }
    #endregion
